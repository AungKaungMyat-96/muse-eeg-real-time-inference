...
module conv3_tb;
    ...
    always @(posedge clk) begin
        if (rst_n && out_valid) begin
            if (conv3_compared < OUTPUT_COUNT) begin
                if ($signed(out_data) !== expected_mem[conv3_compared]) begin
                    mismatch_count <= mismatch_count + 1;
                    if (first_mismatch_reported == 0) begin
                        first_mismatch_reported <= 1;
                        $display("FIRST_CONV3_MISMATCH idx=%0d t=%0d f=%0d rtl=%0d expected=%0d",
                                 conv3_compared,
                                 out_t_idx,
                                 out_f_idx,
                                 $signed(out_data),
                                 expected_mem[conv3_compared]);
                    end
                end
                conv3_compared <= conv3_compared + 1;
            end
        end
    end
    ...
    initial begin
        ...
        fd = $fopen("D:/Users/Lenovo/project_1/muse_cnn_sources/data/conv3_input.mem", "r");
        if (fd == 0) begin
            $display("FAIL: cannot open conv3_input.mem");
            $finish;
        end
        i = 0;
    ...
    initial begin
        cycle_count = 0;
        timeout_cycles = 12000000;
        while (cycle_count < timeout_cycles) begin
            @(posedge clk);
            cycle_count = cycle_count + 1;
            if (conv3_compared == OUTPUT_COUNT) begin
                if (mismatch_count == 0) begin
                    $display("PASS: conv3_compared=48000 mismatches=0");
                end else begin
                    $display("FAIL: conv3_compared=%0d mismatches=%0d", conv3_compared, mismatch_count);
                end
                $finish;
            end
        end

        $display("FAIL: timeout before conv3 completion. conv3_compared=%0d mismatches=%0d",
                 conv3_compared, mismatch_count);
        $finish;
    end
